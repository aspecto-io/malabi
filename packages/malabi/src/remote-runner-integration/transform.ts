...
import { SpanAttributes, Link, SpanKind, SpanStatus, TimedEvent, TraceState } from '@opentelemetry/api';
import * as core from '@opentelemetry/core';
import { COLLECTOR_SPAN_KIND_MAPPING, opentelemetryProto } from '@opentelemetry/exporter-collector/build/src/types';
 ...
 * @param attributes
 */
export function toCollectorAttributes(attributes: SpanAttributes): opentelemetryProto.common.v1.KeyValue[] {
 ...
 * @param values
 */
export function toCollectorArrayValue(values: unknown[]): opentelemetryProto.common.v1.ArrayValue {
    return {
 ...
 * @param attributes
 */
export function toCollectorKeyValueList(attributes: SpanAttributes): opentelemetryProto.common.v1.KeyValueList {
    return {
 ...
 */
export function toCollectorAttributeKeyValue(key: string, value: unknown): opentelemetryProto.common.v1.KeyValue {
 ...
 * @param value
 */
export function toCollectorAnyValue(value: unknown): opentelemetryProto.common.v1.AnyValue {
 ...
 */
export function toCollectorEvents(timedEvents: TimedEvent[]): opentelemetryProto.trace.v1.Span.Event[] {
 ...
 */
function toCollectorLinks(span: ReadableSpan, useHex?: boolean): opentelemetryProto.trace.v1.Span.Link[] {
 ...
 * @param useHex - if ids should be kept as hex without converting to base64
 */
export function toCollectorSpan(span: ReadableSpan, useHex?: boolean): opentelemetryProto.trace.v1.Span {
    return {
 ...
 * @param status
 */
export function toCollectorStatus(status: SpanStatus): opentelemetryProto.trace.v1.SpanStatus {
 ...
 * @param kind
 */
export function toCollectorKind(kind: SpanKind): opentelemetryProto.trace.v1.Span.SpanKind {
 ...
 * @param spans spans
 * @param serviceName service name to set as attribute
 * @param attributes additional attributes to include in the message
 * @param useHex - if ids should be kept as hex without converting to base64
 */
...
function toCollectorResourceSpans(
    ...
    return Array.from(groupedSpans, ([resource, libSpans]) => {
        return {
            ...
            instrumentationLibrarySpans: Array.from(libSpans, ([instrumentationLibrary, spans]) =>
                toCollectorInstrumentationLibrarySpans(instrumentationLibrary, spans, useHex)
            ),
        };
    });
}
