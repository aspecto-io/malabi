...
describe('auto-instrumentation', () => {
    ...
    describe('http', () => {
        ...
        it('outgoing http', async () => {
            ...
            const requestHeaders = JSON.parse(outgoingHttpSpan.attributes[HttpExtendedAttribute.HTTP_REQUEST_HEADERS] as string);
            expect(requestHeaders['traceparent']).toMatch(/^[0-9a-f]{2}-[0-9a-f]{32}-[0-9a-f]{16}-[0-9a-f]{2}$/i);

            const responseHeaders = JSON.parse(outgoingHttpSpan.attributes[HttpExtendedAttribute.HTTP_RESPONSE_HEADERS] as string);
